...
def convert_tensor(key, d):
    d[key] = 1.0 - torch.from_numpy(np.array(d[key], np.float32, copy=False))\
        .transpose(0, 1).contiguous().view(1, d[key].size[0], d[key].size[1])
    return d
...
def load_class_images(d):
    if d['class'] not in OMNIGLOT_CACHE:
        ...
        if len(class_images) == 0:
            raise Exception("No images found for omniglot class {} at {}. Did you run download_omniglot.sh first?"
